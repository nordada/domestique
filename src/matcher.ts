...
export function matchShow(parsed: ParsedName, config: ShowsConfigFile): MatchResult {
  ...
  //
  // Type defaults to "stage-race" whenever a stage OR generic episode number
  // was actually parsed, rather than always "one-day" - a one-day show
  // always files as E01 with no title, which would otherwise silently
  // collapse every stage/episode of an unrecognized multi-part release onto
  // the same filename (this is exactly what happened with Tour de Suisse
  // Women's highlights before this show had its own config entry: stage
  // number got discarded and later stages collided with/overwrote earlier
  // ones - and again with "Tour de Celeb - Episode N", a non-race show whose
  // numbering uses "Episode" rather than "Stage").
  const guessedName =
  ...
  const newShow: ShowConfig = {
    ...
    type: parsed.stageNum !== null || parsed.episodeNum !== null ? "stage-race" : "one-day",
