...
# match-overrides.json/archive-state.json, plus the torrent-registry
# directory - tiny, so doing it every boot is cheap, and
...
if [ "$(id -u)" = "0" ] && [ -n "${PUID:-}" ]; then
  PGID="${PGID:-$PUID}"
  for f in "${CONFIG_PATH:-/app/config/events.json}" "${SETTINGS_PATH:-/app/config/settings.json}" "${ACTIVITY_PATH:-/app/config/activity.json}" "${DEDUPE_STATE_PATH:-/app/config/dedupe-state.json}" "${VERIFY_STATE_PATH:-/app/config/verify-state.json}" "${MATCH_OVERRIDES_PATH:-/app/config/match-overrides.json}" "${ARCHIVE_STATE_PATH:-/app/config/archive-state.json}"; do
